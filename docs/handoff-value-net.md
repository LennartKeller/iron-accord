...
setup every number below was measured on:

| series | W-L-D | rate | s/match |
|---|---|---|---|
| mirror greedy v greedy | 27-27-10 | 0.500 | 2.7 |
| mirror normalai v normalai | 29-29-6 | 0.500 | 3.6 |
| normalai v greedy | 57-5-2 | **0.906** | 1.6 |
| normalai v plain planner | 55-9-0 | 0.859 | 7.3 |
| normalai v base2-net | 42-19-3 | **0.680** | 25.5 |

Both mirrors land exactly on 0.500, so the harness is unbiased and the port
carries no seat bias. base2-net -- the best net this project has trained --
scores 0.867 against that same greedy baseline. NormalAi scores 0.906 against
it and then beats base2-net directly at 0.680, roughly 2.9 sigma, while using
about a sixteenth of the time per match.

That is the external referent none of the earlier numbers had. 0.867 was never
"strong"; it was below a conventional opponent that took a day to port. Read
every rate below with that scale in mind: they compare nets to each other, not
to anything outside this project.

Two consequences worth taking seriously before more training:

- NormalAi is a far better benchmark opponent than greedy, and a far better
  teacher than the planner was. If expert iteration is retried, its games are
  the obvious data source -- with the caveat below.
- The diversity finding still governs. NormalAi is one fixed policy, so its
  self-play will converge the same way the planner's did. `randomizeConfig`
  (ported from CoreAI::randomizeIni) exists precisely to turn it into a family
  of distinct opponents, and that is the form worth generating from.

**Expert iteration does NOT pay here, and the reason is instructive.** Run
properly -- volume held constant so quality is the only variable, non-replaying
games discarded, generated by a net that had passed the probe -- strength falls
monotonically with the share of planner-generated data:

| new-data share | v greedy | v the net that generated it |
|---|---|---|
| 0% (baseline) | **0.867** | — |
| 19.3% (buffer) | 0.820 | 0.430 |
| 100% (pure) | 0.719 | 0.242 |

So the planner's games are WORSE training data than the heuristic's, although
the planner is much the stronger player. Planner self-play converges: both sides
play similar strong lines, so the position distribution narrows, and epsilon at
PLAN level perturbs a whole turn while producing fewer distinct situations than
epsilon at ACTION level does.

That is the same lesson this project has learned three times now. Breadth beat
volume (87 maps beat more games on 65). Counterfactuals beat determinism
(epsilon-greedy beat deterministic self-play). And now diversity beats play
quality: for learning a value function, where the positions come from matters
more than how well they were played.

Do not run another iteration expecting compounding. If expert iteration is
retried, the thing to change is position DIVERSITY -- more maps, more
exploration at action level, wider openings -- not the strength of the generator.

**Superseded: an earlier iteration reported +0.039 and was called a weak
success.** Its buffer carried 23% more data than the net it was compared with,
so the gain was volume, not quality. That is why this run held volume constant.

**Expert iteration, the earlier and confounded attempt.** Iteration 1:
